...
const verifyUserJWT = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const { rows } = await pool.query('SELECT id, full_name, email FROM users WHERE id = $1', [decodedToken.id]);
        ...
        req.user = rows[0]; // Attaching user data to request object
        next();
    } catch (error: any) {
